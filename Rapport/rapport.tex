...
La m\'ethode de reconnaissance faciale Eigenface a pour particularit\'e de se baser sur des \og eigen vectors\fg, c'est-\`a-dire des vecteurs propres. Elle a \'et\'e introduite en 1991 par Turk et Pentland\\
...
Puis on trouve la matrice de projection (P) en r\'esolvant\\ $$M_i*x = \lambda*M_e*x$$\\
Pour trouver la meilleure projection, il suffit de r\'esoudre\\ $$\frac{P^-1*M_e*P}{P^-1*M_i*P}=argmax(P)$$\\
...
Pour plus de d\'etails~: //--BIBLIOGRAPHIE--//\url{http://docs.\textit{OpenCV}.org/modules/contrib/doc/facerec/facerec_tutorial.html#fisherfaces}.\\
...
Cet algorithme est tr\`es utilis\'e comme nous pouvons le voir dans~: //--BIBLIOGRAPHIE--// expression recognition based on Local Binary Patterns:A comprehensive study de Caifeng Shan a, * , Shaogang Gong b , Peter W. McOwan b, o\`u il est combin\'e avec Adaboost (plugin d'\textit{OpenCV}) pour obtenir de meilleurs r\'esultats, et un r\'eseau neuronnal. Mais aussi dans~: Face Recognition with Local Binary Patterns, Spatial Pyramid Histograms and Naive Bayes Nearest Neighbor (NBNN) classification de Daniel Maturana, Domingo Mery and Alvaro Soto o\`u les auteurs essayent d'am\'eliorer l'algorithme dans le but d'en cr\'eer un nouveau~: NBNN. Nous n'utiliserons pas cet algorithme par manque de temps, mais \'egalement car LBPH est d\'ej\`a pr\'esent dans \textit{OpenCV}.\\
...
\subsection{Diff\'erentes solutions}
	Avant toute chose, nous \'etudions les expressions faciales et non directement les \'emotions. Les expressions faciales sont br\`eves~: entre 250ms et 5s. [cf automaticFacialExpressionAnalysis]. Il existe diff\'erentes façons de r\'ecup\'erer des expressions faciales~: l'approche holistique (c'est-\`a-dire que la tête est analys\'ee de façon globale) ou l'approche locale qui consiste \`a prendre des parties de la tête et analyser ces parties ind\'ependamment les unes des autres. \\

Ensuite, pour \og trouver\fg les \'emotions, il existe \'egalement diff\'erentes mani\`eres de proc\'eder. Nous avons l'approche bas\'ee sur un mod\`ele~: c'est la technique utilis\'ee pr\'ec\'edemment pour effectuer la reconnaissance des visages, mais \'a la place de s\'eparer la base de donn\'ees par individus, on s\'epare les image par individus et par \'emotions. On peut imaginer par exemple une arborescence avec un dossier nomm\'e Xavier contenant les sous-dossiers Xavier_Content, Xavier_Peur, Xavier_Degout, ... Le probl\`eme de cette m\'ethode est qu'il faut que l'individu soit connu dans la base de donn\'ees pour reconna\^itre les \'emotions (ce qui explique pourquoi nous n'avons pas choisi cette m\'ethode).  \`a partir d'une image, //-- \`a compl\'eter --////--SEB~: En gros, c'est pas comme ce qu'on a fait pour reconnaitre un visage, mais \`a la place de reconnaitre une personne, c'\'etait une \'emotion. Genre dans la BDD on aurait Seb content, seb pas content, seb dodo, ...--//. Nous avons aussi l'approche bas\'ee sur l'image~: \`a partir d'une image, nous essayons de trouver des expressions faciales qui s'apparentent aux \'emotions primaires d\'efinies par Ekman et Friesen. Tous les humains ont la même façon d'exprimer ces \'emotions primaires, peu importe son origine ou sa culture [cf 2002ElfenbeinMeta - On the Universality and cultural Specificity of Emotion Recognition: A Meta-Analysis]. Cette m\'ethode pr\'esente l'avantage d'être plus simple et plus rapide \`a mettre en place. Cependant, elle est moins robuste, notamment aux changements de position de la tête. \\
...
% /!\ TEST
entr\'ee biblio \cite{bimodalEmotionRecognition} \cite{automaticFacialExpressionAnalysis} \cite{drivingEmotionalExperienceAutomotiveDesign} \cite{universalityCulturalSpecificityEmotionRecognitionMetaAnalysis} \cite{abnormalRecognitionFacialExpression} \cite{authenticFacialExpressionAnalysis} \cite{emotionRecognitionNeurofuzzyNetwork} \cite{emotionRecognitionCauchyNaiveBayes} \cite{facialEmotionrecognitionUsingMultiModalInformation} \cite{realisticFacialExpressionSynthesisImageBasedTalkingHead} \cite{facialExpressionMegamix} \cite{facialExpressionRecognitionFromVideoSequences} \cite{facialExpressionRecognitionLocalBinaryPatterns} \cite{analyseInterpretationMouvementsHumains} \cite{affectiveIntelligentCarInterfacesEmotionRecognition} \cite{improvingAutomotiveSafetyByPairingDriverEmotionAndCarVoiceEmotion} \cite{realTimeFacialFeaturePointsTrackingPyramidalLucasKanadeAlgorithm} \cite{reconnaissanceExpressionsFacialesMemoireAssociativeBidirectionnelleFonctionSortieChaotique} \cite{emotionRecognitionInHumanComputerInteraction} \cite{eigenfaceRecognition} \cite{faceRecognitionLocalBinaryPatternsSpatialPyramidHistogramsNaiveBayes} \cite{photorealisticExpressiveTextTalkingHeadSynthesis} \cite{essentielSecuriteRoutiere} \cite{analyseQuantificationComportementsConducteursAutomobilesPreCrash}
% /test
